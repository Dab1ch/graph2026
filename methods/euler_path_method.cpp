...
int EulerPathMethodHelper(const nlohmann::json& input,
    nlohmann::json* output);

int EulerPathMethod(const nlohmann::json& input,
    nlohmann::json* output) {
  ...
  if (graphType == "Graph") {
    ...
    if (weightType == "int") {
      return EulerPathMethodHelper<WeightedGraph<int>>(input,
          output);
    } else {
      return -1;
    }
...
int EulerPathMethodHelper(const nlohmann::json& input,
    ...
    *output = nlohmann::json::parse("{}");
    ...
    for(int i = 0; i < m; i++){
        g[std::find(vertices.begin(), vertices.end(), edges[i]["from"]) - vertices.begin()][std::find(vertices.begin(), vertices.end(), edges[i]["to"]) - vertices.begin()]++;
        g[std::find(vertices.begin(), vertices.end(), edges[i]["to"]) - vertices.begin()][std::find(vertices.begin(), vertices.end(), edges[i]["from"]) - vertices.begin()]++;
    }
